...
- [ ] Billing API authentication and permission requirements are identified through real requests, including the observable behavior for missing scopes, insufficient roles, and unsupported plans.
- [ ] AI Credits, premium-request or legacy usage measures, included allowance, overage, units, and billing period are never conflated during normalization.
...
- [ ] Preview SDK quota data, if evaluated, is isolated as experimental and its stability, permissions, and divergence from the billing source are recorded.
- [ ] Rate limits, authorization failures, unavailable endpoints, malformed responses, and schema changes produce distinct diagnosable outcomes.
